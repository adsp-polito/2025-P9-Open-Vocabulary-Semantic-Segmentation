...
TIPS_MEAN = (0.5, 0.5, 0.5)
TIPS_STD  = (0.5, 0.5, 0.5)
...
def load_model(gsnet_config, gsnet_weights, finetune_ckpt, tips_model_id, device):
    print("Loading GSNet ...")
    ...
    print(f"Loading TIPSv2 from {tips_model_id} ...")
    tips_model = AutoModel.from_pretrained(tips_model_id, trust_remote_code=True).to(device)
    tips_model.eval()
...
def evaluate_dataset(dataset_name, model_parts, output_dir, amp, device):
    ...
    native_res = _tips_native_res(model_parts["tips_model"])
    ...
    for i, entry in enumerate(dataset_dicts):
        ...
        mean = torch.tensor(TIPS_MEAN).view(3, 1, 1)
        std  = torch.tensor(TIPS_STD).view(3, 1, 1)
        ...
        with torch.no_grad():
            with autocast(enabled=amp):
                logit = tips_inference(
                    ...
                    tips_model=model_parts["tips_model"],
    ...
    print(f"\n{'='*60}")
    print(f"  TIPSv2 GS-Distill | {dataset_name}  mIoU={miou:.4f}")
    print(f"{'='*60}")
...
def parse_args():
    ...
    p.add_argument("--output-dir",     default="output/ashie/tips/eval")
    p.add_argument("--amp",            action="store_true")
    p.add_argument("--device",         default="cuda" if torch.cuda.is_available() else "cpu")
    p.add_argument("--wandb-project",  default="gs-distill")
    p.add_argument("--wandb-run",      default="eval-tips-finetune")
...
def main():
    ...
    print(f"\n{'='*60}")
    print("  SUMMARY — TIPSv2 GS-Distill")
    print(f"{'='*60}")
