#!/usr/bin/env python3
"""
Evaluate the SigLIP GS-Distill finetuned model on all 4 benchmark datasets:
  Potsdam, FloodNet, FLAIR, FAST

Zero-shot: text features built from each dataset's class list using teacher CLIP.
Student + RIPD weights loaded from finetune_best.pth (output of train_siglip.py).

Usage:
    export DETECTRON2_DATASETS='gs_net/data/datasets'
    export RSIB_CKPT='path/to/dinov3.pth'

    python scripts/eval_siglip.py \\
        --gsnet-config  configs/vitl_336_dinov3.yaml \\
        --gsnet-weights output/gsnet_pretrain/model_final.pth \\
        --finetune-ckpt output/ashie/siglip/finetune_best.pth \\
        --output-dir    output/ashie/siglip/eval \\
        [--amp]
"""
...
def remove_gsnet_clip_cache_hooks(gsnet):
    """Remove GSNet's permanent CLIP image hooks; eval uses explicit hooks."""
...
def release_unused_gsnet(gsnet):
    """Drop GSNet container references after extracting eval modules."""
...
def load_model(gsnet_config, gsnet_weights, finetune_ckpt, siglip_model_id, device):
    print("Loading GSNet ...")
    ...
    print(f"Loading SigLIP from {siglip_model_id} ...")
    ...
    backbone_dim = siglip_model.config.vision_config.hidden_size
    clip_skip_dims = (
    ...
    clip_skip_indices = tuple(student_args.get("siglip_skip_layers", [3, 7]))
    student = SigLIPStudent(
        siglip_model=siglip_model,
        d_dino=student_args.get("d_dino", 768),
        siglip_layers=student_args.get("siglip_layers", SIGLIP_LAYERS_DEFAULT),
...
def evaluate_dataset(dataset_name, model_parts, output_dir, amp, device):
    ...
    # Text features from teacher CLIP (same embedding space RIPD was trained with)
    text_feats = build_text_features(cfg["class_json"], model_parts["clip_model"], device)

    native_res = _siglip_native_res(model_parts["siglip_model"])
    ...
    for i, entry in enumerate(dataset_dicts):
        ...
        img_resized = img_pil.resize((native_res, native_res), Image.BILINEAR)
        img_t = to_tensor(img_resized)
        mean = torch.tensor(SIGLIP_MEAN).view(3, 1, 1)
        std  = torch.tensor(SIGLIP_STD).view(3, 1, 1)
        ...
        with torch.no_grad():
            with autocast(enabled=amp):
                logit = siglip_inference(
                    ...
                    siglip_model=model_parts["siglip_model"],
                    ...
                    skip_layer_indices=model_parts["clip_skip_indices"],
                )
    ...
    print(f"\n{'='*60}")
    print(f"  SigLIP GS-Distill | {dataset_name}  mIoU={miou:.4f}")
    print(f"{'='*60}")
...
def main():
    ...
    model_parts = load_model(
        args.gsnet_config, args.gsnet_weights, args.finetune_ckpt,
    ...
    print(f"\n{'='*60}")
    print("  SUMMARY — SigLIP GS-Distill")
    print(f"{'='*60}")
