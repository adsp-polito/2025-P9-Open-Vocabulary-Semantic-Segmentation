...
# Phase 3 training skipped — checkpoint already at output/ashie/finetune/finetune_best.pth
# if [ "${1:-}" = "--after" ] && [ -n "${2:-}" ]; then
...
EVAL_JOB=$(sbatch --parsable jobs-ashie/eval_clip.sbatch)
echo "Eval submitted:     job ${EVAL_JOB}"
