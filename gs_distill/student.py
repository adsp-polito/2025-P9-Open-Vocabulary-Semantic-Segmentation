...
Architecture:
  ...
  - Branch 1 (fusion):    predicts fused_corr_embed  shape (B, hidden_dim, T, 24, 24)
  - Branch 2 (dino_L4):   predicts DINOv3 layer-4 features shape (B, 768, 48, 48)
  - Branch 3 (dino_L8):   predicts DINOv3 layer-8 features shape (B, 768, 48, 48)
...
class GSDistillStudent(nn.Module):
    def __init__(
        ...
        # Branch 1: predict fused_corr_embed (B, hidden_dim, T, 24, 24)
        # We predict hidden_dim * num_classes channels at 24x24, then reshape.
        self.fusion_branch = nn.Conv2d(
            TRUNK_OUT, hidden_dim * num_classes, kernel_size=3, padding=1
        )

        # Branch 2: predict dino_L4 (B, d_dino, 48, 48) — upsample 2x from 24->48
        ...
        # Branch 3: predict dino_L8 (B, d_dino, 48, 48)
    ...
    def forward(self, image: torch.Tensor) -> dict:
        ...
        # Branch 1: fused_corr_embed
        fusion_flat = self.fusion_branch(trunk_out)          # (B, hidden_dim*T, 24, 24)
        fused_corr_embed = rearrange(
            fusion_flat,
            "B (C T) H W -> B C T H W",
            C=self.hidden_dim,
            T=self.num_classes,
