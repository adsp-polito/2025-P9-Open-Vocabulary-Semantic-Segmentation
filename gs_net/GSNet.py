...
def BuildRSIB(Weights):
    ...
    - Employs ViT architecture matching CLIP for seamless integration
    ...
    if Weights is None:
        raise ValueError(
            "RSIB checkpoint path is None. Please set the RSIB_CKPT environment variable:\n"
            "export RSIB_CKPT='path/to/dinov3_vitl16_pretrain_sat493m-eadcf0ff.pth'"
        )
    ...
    model = model.float()
    print("✓ DINOv3 RSIB successfully built")
    return model
...
class GSNet(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        if self.dino_model is not None:
            # DINOv3Wrapper.get_intermediate_layers returns 12 features with shape (B, 2305, 768)
            # Structure: [CLS token (1), patch tokens (2304 = 48×48)]
            # The wrapper already handles:
            # - Dimension projection: 1024 → 768
            # - Spatial upsampling: 24×24 → 48×48
            # - Register token removal
