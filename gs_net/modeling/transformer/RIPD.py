...
class RIPD(nn.Module):
    ...
    def forward_from_fusion(self, fused_corr_embed, text_feats, appearance_guidance, dino_guidance):
        """
        GS-Distill inference entry-point: skip QGFF and feed the student-predicted
        fused_corr_embed directly into the AggregatorLayers + decoder.

        Args:
            fused_corr_embed:   (B, hidden_dim, T, H, W) — student branch-1 output.
            text_feats:         (B, T, P, C) — CLIP text embeddings.
            appearance_guidance: dict with keys 'res3', 'res4', 'res5' mapping to
                                 (B, C, H, W) CLIP intermediate features, or a tuple
                                 where index 0 is the main guidance and 1: are decoder skips.
            dino_guidance:      list of two (B, C, H, W) projected DINOv3 skip tensors.

        Returns:
            logit: (B, T, H, W)
        """
        # Normalise appearance_guidance to tuple form that the projection layers expect
        if isinstance(appearance_guidance, dict):
            # Convert dict → list in the order (res3 for global, res4, res5 for decoder)
        ...
        if self.guidance_projection is not None and app_guid_tuple[0] is not None:
            projected_guidance = self.guidance_projection(app_guid_tuple[0])

        if self.CLIP_decoder_guidance_projection is not None:
            clip_decoder_skips = app_guid_tuple[1:]
            CLIP_projected_decoder_guidance = [
                proj(g) for proj, g in zip(self.CLIP_decoder_guidance_projection, clip_decoder_skips)
                if g is not None
            ]
            # Pad to length 2 if needed
            while len(CLIP_projected_decoder_guidance) < 2:
                CLIP_projected_decoder_guidance.append(None)
        ...
        if self.DINO_decoder_guidance_projection is not None and dino_guidance is not None:
            ...
            while len(DINO_projected_decoder_guidance) < 2:
                DINO_projected_decoder_guidance.append(None)
